...
async function main() {
  ...
  async function ensurePost(slugs: { en: string; ml: string }, data: {
    date?: Date;
    imageUrl?: string;
    isAd?: boolean;
    ctaUrl?: string | null;
    categorySlug?: string | null;
    en: { title: string; excerpt?: string; content?: string; adText?: string | null; ctaText?: string | null };
    ml: { title: string; excerpt?: string; content?: string; adText?: string | null; ctaText?: string | null };
  }) {
    const existingEn = await prisma.postTranslation.findUnique({
      where: { slug_locale: { slug: slugs.en, locale: 'en' } },
    });
    if (existingEn) return; // already seeded
  ...
  await ensurePost(
    { en: 'sustainable-homestead-design', ml: 'sustainable-homestead-design-ml' },
    {
  ...
  await ensurePost(
    { en: 'water-conservation-in-agriculture', ml: 'water-conservation-in-agriculture-ml' },
    {
      categorySlug: 'water',
  ...
  await ensurePost(
    { en: 'premium-agricultural-tools', ml: 'premium-agricultural-tools-ml' },
    {
      isAd: true,
