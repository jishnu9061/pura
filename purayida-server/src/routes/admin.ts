  ...
  try {
    ...
    try {
      ...
      if (SMTP_HOST && SMTP_PORT && order.email) {
        ...
        const itemsText = (order.items || []).map((it: any) => `${it.book?.translations?.[0]?.name || `#${it.bookId}`} x ${it.quantity}`).join('\n');
...
router.post('/services', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    if (String(err.message || '').includes('Unique constraint')) return res.status(409).json({ error: 'Slug already exists for locale' });
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.get('/book-comments', requireRole('EDITOR'), async (_req, res) => {
  const items = await (prisma as any).bookComment.findMany({ orderBy: { createdAt: 'desc' }, include: { book: { include: { translations: { take: 1 } } } } });
  res.json(items);
});
...
router.delete('/book-comments/:id', requireRole('EDITOR'), async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid id' });
  await (prisma as any).bookComment.delete({ where: { id } });
  res.status(204).send();
});
...
router.get('/services-page', requireRole('EDITOR'), async (req, res) => {
  const locale = (req.query.locale as string) === 'ml' ? 'ml' : 'en';
...
router.patch('/gallery-groups/:id', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    if (parsed.date !== undefined) data.date = parsed.date ? new Date(parsed.date) : null;
...
router.patch('/gallery-images/:id', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    if (parsed.videoUrl !== undefined) data.videoUrl = parsed.videoUrl;
    if (parsed.youtubeUrl !== undefined) data.youtubeUrl = parsed.youtubeUrl;
...
router.get('/settings', requireRole('EDITOR'), async (_req, res) => {
  try {
    ...
    if (!settings) settings = await prisma.siteSettings.create({ data: { id: 1, defaultLocale: 'en' } });
    res.json(settings);
  } catch (err) {
    console.error(err);
...
router.patch('/settings', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    const update: any = {};
    if (data.heroBackgroundUrl !== undefined) update.heroBackgroundUrl = data.heroBackgroundUrl || null;
    if (data.agroecologyBackgroundUrl !== undefined) update.agroecologyBackgroundUrl = data.agroecologyBackgroundUrl || null;
    if (data.blogCommentsEmail !== undefined) update.blogCommentsEmail = data.blogCommentsEmail || null;
    if (data.bookOrdersEmail !== undefined) update.bookOrdersEmail = data.bookOrdersEmail || null;
    if (data.heroBox2Slug !== undefined) update.heroBox2Slug = data.heroBox2Slug || null;
    if (data.heroBox3Slug !== undefined) update.heroBox3Slug = data.heroBox3Slug || null;
...
router.get('/ads/metrics', requireRole('EDITOR'), async (_req, res) => {
  try {
    ...
    for (const ad of recent) {
      ...
      if (!hasEn) missingEn++;
      if (!hasMl) missingMl++;
      if (!ad.imageUrl) missingImage++;
      if (!ad.ctaUrl) missingCta++;
      if (ad.updatedAt >= sevenDaysAgo) updated7d++;
      if (ad.createdAt >= sevenDaysAgo) created7d++;
    }
...
async function ensureUniqueServiceSlug(desired: string, locale: 'en' | 'ml', serviceId: number): Promise<string> {
  let base = desired;
  let candidate = base;
  let n = 1;
...
async function ensureUniqueServiceSlugForCreate(desired: string, locale: 'en' | 'ml'): Promise<string> {
  let base = desired;
  let candidate = base;
  let n = 1;
  // eslint-disable-next-line no-constant-condition
...
router.post('/categories', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    if (err instanceof z.ZodError) return res.status(400).json({ error: 'Invalid payload', details: err.issues });
    if (String(err.message || '').includes('Unique constraint')) return res.status(409).json({ error: 'Slug already exists' });
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.patch('/posts/:id/translations', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    for (const t of translations) {
      const data = { title: t.title, excerpt: t.excerpt ?? undefined, content: t.content ?? undefined, adText: t.adText ?? undefined, ctaText: t.ctaText ?? undefined, slug: t.slug, metaTitle: t.metaTitle ?? undefined, metaDescription: t.metaDescription ?? undefined, categoryLabel: (t as any).categoryLabel ?? undefined } as any;
...
router.patch('/settings', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    const data: any = {};
    if (parsed.heroBackgroundUrl !== undefined) data.heroBackgroundUrl = parsed.heroBackgroundUrl;
    if (parsed.agroecologyBackgroundUrl !== undefined) data.agroecologyBackgroundUrl = parsed.agroecologyBackgroundUrl;
    if (parsed.blogCommentsEmail !== undefined) data.blogCommentsEmail = parsed.blogCommentsEmail;
    if (parsed.bookOrdersEmail !== undefined) data.bookOrdersEmail = parsed.bookOrdersEmail;
...
router.post('/send-email', requireRole('EDITOR'), async (req, res) => {
  try {
    ...
    const { SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS } = process.env as any;
    if (!SMTP_HOST || !SMTP_PORT) return res.status(500).json({ error: 'Email not configured' });
    const transporter = nodemailer.createTransport({
      host: SMTP_HOST,
