...
router.get('/posts', async (req, res) => {
  try {
    const locale = getLocale(req.query.locale as string);
    const categorySlug = (req.query.category as string) || undefined;
    const page = Math.max(1, parseInt((req.query.page as string) || '1', 10));
    let pageSize = Math.min(50, Math.max(1, parseInt((req.query.pageSize as string) || '9', 10)));
    let skip = (page - 1) * pageSize;

    // Optional ads filter and limit for simple ad feeds
    const isAdParam = (req.query.isAd as string) || undefined;
    const placementParam = (req.query.placement as string) || undefined; // home | blog_all | blog_detail | all
    const blogSlugParam = (req.query.blogSlug as string) || undefined;
    const limitParam = (req.query.limit as string) || undefined;
    let isAd: boolean | undefined = undefined;
    if (isAdParam === 'true') isAd = true;
    else if (isAdParam === 'false') isAd = false;
    if (limitParam) {
...
async function renderShareHtml(slug: string, locale: 'en' | 'ml', req: any) {
  const t = await prisma.postTranslation.findFirst({
    where: { slug, locale },
    include: { post: true },
  });
  if (!t || !t.post || !t.post.published) return null;
...
router.get('/share/:locale/:slug', async (req, res) => {
  try {
    const slug = String(req.params.slug);
    const locale = (req.params.locale === 'ml' ? 'ml' : 'en') as 'en' | 'ml';
    const html = await renderShareHtml(slug, locale, req);
    if (!html) return res.status(404).send('Not found');
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(html);
    ...
    const whereClause: any = { categoryId: categoryId ?? undefined, published: true };
    if (isAd !== undefined) {
    ...
    const items = posts.map((p) => {
      ...
      return {
        ...
        categoryLabel: (t as any)?.categoryLabel ?? null,
        date: p.date,
        imageUrl: p.imageUrl,
        isAd: p.isAd,
        ctaUrl: p.ctaUrl,
        category: p.category ? { slug: p.category.slug, name: (p.category as any).translations?.[0]?.name ?? p.category.slug } : null,
      };
    });
...
router.get('/ads', async (req, res) => {
  try {
    ...
    const whereClause: any = { isAd: true, published: true };
    if (zone) whereClause.adZone = zone;
...
router.get('/posts/:slug', async (req, res) => {
  try {
    ...
    if (!t || !t.post.published) return res.status(404).json({ error: 'Not found' });
    ...
    res.json({
      ...
      category: t.post.category ? { slug: t.post.category.slug, name: (t.post.category as any).translations?.[0]?.name ?? t.post.category.slug } : null,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
...
router.get('/gallery-sets', async (req, res) => {
  try {
    ...
    for (const img of images) {
      const type = (img as any).type ?? ((img as any).youtubeUrl ? 'youtube' : ((img as any).videoUrl ? 'video' : 'image'));
      const t = img.translations?.[0];
      const mapped = {
        id: img.id,
        imageUrl: img.imageUrl ?? null,
        videoUrl: (img as any).videoUrl ?? null,
        youtubeUrl: (img as any).youtubeUrl ?? null,
        type,
        caption: t?.caption ?? '',
        description: t?.description ?? '',
        order: img.order,
      };
      if (!byGroup[img.groupId!]) byGroup[img.groupId!] = [];
...
router.get('/services/:slug', async (req, res) => {
  try {
    ...
    if (!service || !t) return res.status(404).json({ error: 'Not found' });
    ...
    res.json({
      ...
      order: service.order,
      phone: (service as any).phone ?? null,
      email: (service as any).email ?? null,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
...
router.get('/books', async (req, res) => {
  try {
    ...
    const items = books.map((b) => ({
      ...
      page1Url: (b as any).page1Url ?? null,
      page2Url: (b as any).page2Url ?? null,
      quality: (b as any).quality ?? null,
      status: (b as any).status ?? 'INSTOCK',
      stock: (b as any).stock ?? 0,
      isPreorder: (b as any).isPreorder ?? false,
      link: b.link ?? null,
      order: b.order,
    }));
    res.json(items);
...
router.get('/books/:slug', async (req, res) => {
  try {
    ...
    res.json({
      ...
      page1Url: (t.book as any).page1Url ?? null,
      page2Url: (t.book as any).page2Url ?? null,
      quality: (t.book as any).quality ?? null,
      status: (t.book as any).status ?? 'INSTOCK',
      stock: (t.book as any).stock ?? 0,
      isPreorder: (t.book as any).isPreorder ?? false,
...
router.get('/gallery-images', async (req, res) => {
  try {
    ...
    const items = images.map((g) => ({
      ...
      youtubeUrl: (g as any).youtubeUrl ?? null,
      type: (g as any).type ?? ((g as any).youtubeUrl ? 'youtube' : ((g as any).videoUrl ? 'video' : 'image')),
      ...
      try {
        ...
        if (SMTP_HOST && SMTP_PORT && email) {
          ...
          try {
            ...
            console.warn(`[mail] SMTP verify failed for contact ack (${SMTP_HOST}:${port}, secure:${isSSL})`, (ve as any)?.message || ve);
          ...
          try {
            ...
            cc = (settings as any)?.contactEmail || ADMIN_EMAIL || 'hello@purayidakrishi.com';
...
function parsePriceToNumber(p?: string | null) {
  if (!p) return 0;
...
router.post('/orders', orderLimiter, async (req, res) => {
  try {
    ...
    if (books.length !== bookIds.length) return res.status(400).json({ error: 'Invalid book in items' });
    ...
    const hasPreorderBook = books.some((b) => (b as any).isPreorder === true);
    ...
    (async () => {
      try {
        ...
        if (SMTP_HOST && SMTP_PORT) {
          ...
          const itemsText = created.items.map((it: any) => `#${it.bookId} x ${it.quantity} @ ${it.price || ''}`).join('\n');
          const itemsHtml = created.items.map((it: any) => 
            `<tr><td style="padding:8px;border:1px solid #ddd;">Book #${it.bookId}</td><td style="padding:8px;border:1px solid #ddd;">${it.quantity}</td><td style="padding:8px;border:1px solid #ddd;">${it.price || 'N/A'}</td></tr>`
          ).join('');

          // Admin notification email
          const settings = await prisma.siteSettings.findUnique({ where: { id: 1 } });
          const adminTo = (settings as any)?.bookOrdersEmail || settings?.contactEmail || ADMIN_EMAIL || 'hello@purayidakrishi.com';
...
const imageFilter: multer.Options['fileFilter'] = (_req, file, cb) => {
  ...
  if (!allowed.includes(file.mimetype)) return cb(new Error('Only image uploads are allowed'));
  cb(null, true);
};
...
router.get('/books/:id/comments', async (req, res) => {
  ...
  const list = await (prisma as any).bookComment.findMany({ where: { bookId: id, approved: true }, orderBy: { createdAt: 'desc' }, take: 50 });
  res.json(list.map((c: any) => ({ id: c.id, name: c.name, content: c.content, createdAt: c.createdAt })));
});
...
router.post('/books/:id/comments', commentsLimiter, async (req, res) => {
  try {
    ...
    const created = await (prisma as any).bookComment.create({ data: { bookId: id, name: parsed.name, content: parsed.content } });
    ...
    (async () => {
      try {
        ...
        if (SMTP_HOST && SMTP_PORT) {
          ...
          const settings = await prisma.siteSettings.findUnique({ where: { id: 1 } });
          const adminTo = (settings as any)?.bookCommentsEmail || settings?.contactEmail || ADMIN_EMAIL || 'hello@purayidakrishi.com';
...
router.get('/posts/:id/comments', async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid id' });
  const list = await (prisma as any).postComment.findMany({ where: { postId: id, approved: true }, orderBy: { createdAt: 'desc' }, take: 50 });
  res.json(list.map((c: any) => ({ id: c.id, name: c.name, content: c.content, createdAt: c.createdAt })));
});
...
router.post('/posts/:id/comments', postCommentsLimiter, async (req, res) => {
  try {
    ...
    if (name.length > 120 || content.length > 1000) return res.status(400).json({ error: 'Input too long' });
    ...
    const created = await (prisma as any).postComment.create({ data: { postId: id, name, content, approved: true } });
    ...
    (async () => {
      try {
        ...
        if (SMTP_HOST && SMTP_PORT) {
          ...
          const settings = await prisma.siteSettings.findUnique({ where: { id: 1 } });
          const adminTo = (settings as any)?.blogCommentsEmail || settings?.contactEmail || ADMIN_EMAIL || 'hello@purayidakrishi.com';
